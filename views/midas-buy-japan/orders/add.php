<div class="container mt-5">
    ...
    <div class="form-group mt-3">
        ...
        <textarea class="form-control" id="order_paste" name="order_paste" rows="5" placeholder="Dán nội dung đơn hàng (Mã đơn hàng, Tên sản phẩm, UID, Ngày mua)..."></textarea>
...
(function() {
    function parseOrderText(text) {
        var result = { uid: '', card: 'normal', order_id: '', product: '', date: '' };
        if (!text || !text.trim()) return result;

        var lines = text.split('\n');
        for (var i = 0; i < lines.length; i++) {
            var line = lines[i].trim();
            if (line.indexOf('Mã đơn hàng:') === 0) {
                result.order_id = line.replace(/Mã đơn hàng:\s*/i, '').trim();
            } else if (line.indexOf('Tên sản phẩm:') === 0) {
                result.product = line.replace(/Tên sản phẩm:\s*/i, '').trim();
