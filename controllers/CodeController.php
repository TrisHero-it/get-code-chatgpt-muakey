...
class CodeController extends Account
{
    ...
    public function index()
    {
        ...
        // Enrich: lấy link verify (server-side) để không lộ token ở frontend
        $enriched = [];
        foreach ($results as $item) {
            if (!is_array($item)) continue;
            if (!isset($item['_token']) || !$item['_token']) continue;
            if (!isset($item['@id'])) continue;

            // chỉ xử lý mail Netflix gửi tới email đang search (nếu có)
            if (isset($_GET['email']) && $_GET['email'] !== '') {
                $email = strtolower($_GET['email']);
                $fromName = $item['from']['name'] ?? null;
                $toAddr = $item['to'][0]['address'] ?? null;
                if (!($fromName === 'Netflix' && $toAddr && mb_strtolower($toAddr, 'UTF-8') === $email)) {
                    continue;
                }
            }

            $verifyLink = $this->extractNetflixVerifyLink($item['_token'], $item['@id']);
            if ($verifyLink) {
                $item['verifyLink'] = $verifyLink;
                // không cần đưa token sang view nữa
                unset($item['_token']);
                $enriched[] = $item;
            }
        }
    ...
    public function apiSearch(): void
    {
        ...
        $account2 = new Account();
        $account = $account2->getAccountByEmailAndType($email, 'Netflix');
